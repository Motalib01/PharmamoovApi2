...
{
    public class PaymentDeliveryBackgroundProcess : IPaymentDeliveryBackgroundProcess
    {
        ...
        public PaymentDeliveryBackgroundProcess(ILoggerManager _logManager, APIConfigurationManager _apiCon, IServiceScopeFactory factory)
        {
            APIConfig = _apiCon;
        ...
        public void TransferFundsFromShopWalletToDeliveryWallet()
        {
            try
            {
                List<PaymentDelivery> SucceededPayments = DbContxt.PaymentDelivery.Where(pa => pa.Status == EPaymentStatus.CREATED).ToList();
                LogManager.LogInfo("-- Total of " + SucceededPayments.Count() + " payment transaction with success status -- ");
                foreach (PaymentDelivery pay in SucceededPayments)
                {
                    TimeSpan paymentDate = DateTime.Now - (pay.LastEditedDate == null ? DateTime.Now.AddHours(-10) : ((DateTime)pay.LastEditedDate));
                    if (paymentDate.TotalHours > APIConfig.HostedServicesConfig.AutomaticFundsTransferIntervalHrs)
                    {
                        ...
                        if (PaymentOrder != null)
                        {
                            ...
                            DebitedFunds.Currency = pay.DebitedFundsCurrency;
                            DebitedFunds.Amount = (long)pay.DebitedFunds;

                            Money Fees = new Money();
                            Fees.Currency = pay.FeesCurrency;
                            Fees.Amount = (long)pay.Fees;
                            ...
                            if (transferResult.Status == MangoPay.SDK.Core.Enumerations.TransactionStatus.SUCCEEDED)
                            {
                                ...
                                PaymentDelivery PaymentTrans = DbContxt.PaymentDelivery.FirstOrDefault(pt => pt.Id == pay.Id);
